// Join a room by code with enhanced cross-network support
function joinRoomByCode(roomCode) {
    ...
    // Enhanced room joining with cross-network discovery
    if (typeof sendWebSocketMessage === 'function') {
        sendWebSocketMessage({
            type: 'join-public-room',
            roomCode: currentRoomCode,
            crossNetwork: true, // Enable cross-network discovery
            deviceInfo: window.deviceInfo || getSystemInfo(),
            connectionType: getConnectionType() // Detect if using mobile data or WiFi
        });
        console.log('🌐 Joining public room with cross-network support:', currentRoomCode);
        showNotification(`🔗 Connecting to room ${currentRoomCode} (cross-network enabled)...`, 'info');
        
        // Start enhanced peer discovery for cross-network
        startCrossNetworkDiscovery(roomCode);
    } else {
...
// Detect connection type (WiFi vs Mobile Data)
...
// Enhanced cross-network discovery
function startCrossNetworkDiscovery(roomCode) {
    console.log('🔍 Starting cross-network discovery for room:', roomCode);
    
    // Use multiple discovery methods for cross-network
    setTimeout(() => {
        if (typeof sendWebSocketMessage === 'function') {
            sendWebSocketMessage({
                type: 'enhanced-discovery',
                roomCode: roomCode,
                timestamp: Date.now()
            });
        }
    ...
    const discoveryInterval = setInterval(() => {
        if (currentRoomCode === roomCode && typeof sendWebSocketMessage === 'function') {
            sendWebSocketMessage({
                type: 'room-ping',
                roomCode: roomCode,
                timestamp: Date.now()
...
window.startCrossNetworkDiscovery = startCrossNetworkDiscovery;
