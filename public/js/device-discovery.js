...
function connectToSignalingServer() {
    ...
    // Enhanced WebSocket URL determination with cloud support
    let wsUrl;
    
    // Check for cloud deployment
    if (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        // Production/cloud mode - use current host
        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        wsUrl = `${protocol}//${window.location.host}/socket.io/`;
        console.log('🌐 Using cloud signaling server for cross-network support');
    } else {
        // Always connect to P2P server on port 3000 for local development
        // Whether accessed via React (5173) or P2P server (3000)
        wsUrl = 'ws://localhost:3000';
        console.log('🔗 Connecting to local P2P signaling server on port 3000');
    }
...
function handleSignalingMessage(message) {
    switch (message.type) {
        ...
        // Cross-network room management
        case 'public-room-joined':
            console.log('🌐 Successfully joined cross-network room:', message.roomId);
            showNotification(`✅ Connected to room ${message.roomId} (cross-network enabled)`, 'success');
            break;

        case 'room-join-error':
            console.error('❌ Failed to join room:', message.error);
            showNotification(`Failed to join room: ${message.error}`, 'error');
            break;

        case 'enhanced-discovery-response':
            console.log('📡 Enhanced discovery response for room:', message.roomCode);
            handleCrossNetworkPeers(message.peers, message.roomCode);
            break;

        case 'room-pong':
            console.log('🏓 Room pong from peer:', message.fromPeer);
            // Update device info for cross-network peer
            if (message.deviceInfo) {
                updateCrossNetworkPeer(message.fromPeer, message.deviceInfo);
            }
            break;

        case 'cross-network-room-info':
            console.log('🌐 Received cross-network room info:', message);
            handleCrossNetworkRoomInfo(message);
            break;
...
function createDeviceCard(peerId, deviceInfo, isConnected = false) {
    ...
    // Different styling for connected vs nearby devices vs room members vs cross-network
    if (isConnected) {
    ...
    } else if (deviceInfo.crossNetwork) {
        deviceCard.style.border = '2px solid #ff9800';
        deviceCard.style.background = 'linear-gradient(135deg, #fff3e0 0%, #ffeaa7 100%)';
        deviceCard.classList.add('cross-network-item');
    ...
    if (isConnected) {
        ...
        avatar.style.border = '3px solid #ff9800';
        avatar.style.boxShadow = '0 0 10px rgba(255, 152, 0, 0.3)';
        avatar.style.background = 'linear-gradient(135deg, #ff9800 0%, #f57c00 100%)';
    } else if (deviceInfo.isRoomMember) {
    ...
    if (isConnected) {
        ...
        connectionText = `🌐 Cross-network${deviceInfo.roomCode ? ` (Room: ${deviceInfo.roomCode})` : ''}`;
        statusColor = '#ff9800';
    } else if (deviceInfo.isRoomMember) {
    ...
    const networkInfo = deviceInfo.crossNetwork ? ' • Cross-network' : '';
...
// Cross-network helper functions
function handleCrossNetworkPeers(peers, roomCode) {
    console.log(`🌐 Processing ${peers.length} cross-network peers for room ${roomCode}`);
    
    peers.forEach(peer => {
        // Add special marker for cross-network peers
        peer.crossNetwork = true;
        peer.roomCode = roomCode;
        addPeerToNearbyDevices(peer, 'public');
    });
    
    if (peers.length > 0) {
        showNotification(`🔗 Found ${peers.length} device(s) in room ${roomCode}`, 'info');
    }
}

function updateCrossNetworkPeer(peerId, deviceInfo) {
    const existingDevice = nearbyDevices.get(peerId);
    if (existingDevice) {
        // Update device info
        existingDevice.name = deviceInfo.displayName || deviceInfo.name || 'Cross-network Device';
        existingDevice.lastSeen = Date.now();
        existingDevice.crossNetwork = true;
        updateDeviceGrid();
    } else {
        // Add new cross-network device
        const newDevice = {
            id: peerId,
            name: deviceInfo.displayName || deviceInfo.name || 'Cross-network Device',
            type: deviceInfo.deviceName || deviceInfo.type || 'unknown',
            browser: deviceInfo.browser || 'Browser',
            lastSeen: Date.now(),
            crossNetwork: true,
            roomType: 'public'
        };
        nearbyDevices.set(peerId, newDevice);
        updateDeviceGrid();
    }
}

function handleCrossNetworkRoomInfo(message) {
    console.log('🌐 Cross-network room info received:', message);
    
    // Store server info for better connection handling
    window.crossNetworkInfo = {
        roomCode: message.roomCode,
        serverInfo: message.serverInfo,
        stunServers: message.stunServers,
        turnServers: message.turnServers
    };
    
    // Update PeerJS config with additional TURN servers if available
    if (message.turnServers && window.PEER_CONFIG) {
        const existingServers = window.PEER_CONFIG.config.iceServers || [];
        const newServers = message.turnServers.filter(turnServer => 
            !existingServers.some(existing => existing.urls === turnServer.urls)
        );
        
        if (newServers.length > 0) {
            window.PEER_CONFIG.config.iceServers = [...existingServers, ...newServers];
            console.log('🔄 Updated PeerJS config with additional TURN servers');
        }
    }
}
...
window.handleCrossNetworkPeers = handleCrossNetworkPeers;
window.updateCrossNetworkPeer = updateCrossNetworkPeer;
window.handleCrossNetworkRoomInfo = handleCrossNetworkRoomInfo;
